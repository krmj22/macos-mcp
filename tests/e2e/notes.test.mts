...
before(async () => {
  ...
  console.log(
    `  Warmup complete: ${Math.round(performance.now() - warmStart)}ms`,
  );
});
...
after(async () => {
  ...
  for (const id of createdNoteIds) {
    try {
      ...
      console.log(
        `  cleanup: failed to delete note ${id} (may already be deleted)`,
      );
    }
  }
...
describe('Notes CRUD', () => {
  ...
  it('T5: create minimal (title only)', async () => {
    ...
    assert.ok(
      elapsed < TIMEOUT_CREATE,
      `create took ${elapsed}ms (>${TIMEOUT_CREATE}ms)`,
    );
  });
  ...
  it('T6: create with body + folder', async () => {
    const { text, elapsed } = await callTool(
      ...
      {
        action: 'create',
        title: `${PREFIX} WithBody ${uniqueTag}`,
        body,
        folder: 'Notes',
      },
      'CRUD',
  ...
  it('T7: read by ID', async () => {
    ...
    assert.ok(
      elapsed < TIMEOUT_READ,
      `read took ${elapsed}ms (>${TIMEOUT_READ}ms)`,
    );
  });
  ...
  it('T8: update title', async () => {
    ...
    assert.ok(
      elapsed < TIMEOUT_READ,
      `update took ${elapsed}ms (>${TIMEOUT_READ}ms)`,
    );
  });
  ...
  it('T10: append to note', async () => {
    ...
    assert.ok(
      elapsed < TIMEOUT_READ,
      `append took ${elapsed}ms (>${TIMEOUT_READ}ms)`,
    );
    ...
    assert.ok(
      readText.includes('Appended content'),
  ...
  it('T11: move to folder', async () => {
    ...
    assert.ok(
      elapsed < TIMEOUT_READ,
      `move took ${elapsed}ms (>${TIMEOUT_READ}ms)`,
    );
  ...
  it('T12: delete note', async () => {
    ...
    assert.ok(
      text.includes('Recently Deleted'),
      'should mention Recently Deleted',
    );
    assert.ok(
      elapsed < TIMEOUT_READ,
      `delete took ${elapsed}ms (>${TIMEOUT_READ}ms)`,
    );
...
describe('Notes Read + Search', () => {
  ...
  it('T1: read all notes', async () => {
    ...
    assert.ok(
      text.includes('### Notes'),
      `should have Notes header, got: ${text.slice(0, 200)}`,
    );
    assert.ok(
      text.includes('Total:') || text.includes('Showing'),
      'should have count',
    );
    assert.ok(
      elapsed < TIMEOUT_LIST,
      `read all took ${elapsed}ms (>${TIMEOUT_LIST}ms)`,
    );
  });
  ...
  it('T2: read with pagination (limit 5)', async () => {
    ...
    assert.ok(
      elapsed < TIMEOUT_LIST,
      `paginated read took ${elapsed}ms (>${TIMEOUT_LIST}ms)`,
    );
  });
  ...
  it('T3: filter by folder', async () => {
    ...
    if (text.includes('- **')) {
      assert.ok(
        text.includes('Folder: Notes'),
        'items should be in Notes folder',
      );
    }
    assert.ok(
      elapsed < TIMEOUT_FOLDER_LIST,
      `folder filter took ${elapsed}ms (>${TIMEOUT_FOLDER_LIST}ms)`,
    );
  });
  ...
  it('T4: search notes', async () => {
    ...
    const { text: createText } = await callTool(
      ...
      {
        action: 'create',
        title: `${PREFIX} Searchable ${searchTag}`,
        body: 'Unique searchable body',
      },
      'Read',
    );
    ...
    assert.ok(
      elapsed < TIMEOUT_SEARCH,
      `search took ${elapsed}ms (>${TIMEOUT_SEARCH}ms)`,
    );
  });
});
...
describe('Notes Folders', () => {
  ...
  it('T13: read all folders', async () => {
    ...
    assert.ok(
      elapsed < TIMEOUT_FOLDER_LIST,
      `folder read took ${elapsed}ms (>${TIMEOUT_FOLDER_LIST}ms)`,
    );
  });
  ...
  it('T14: create folder', async () => {
    ...
    assert.ok(
      text.includes('Successfully created folder'),
      `unexpected: ${text.slice(0, 200)}`,
    );
    assert.ok(
      elapsed < TIMEOUT_CREATE,
      `folder create took ${elapsed}ms (>${TIMEOUT_CREATE}ms)`,
    );
  });
  ...
  it('T15: create note in new folder', async () => {
    const { text, elapsed } = await callTool(
      ...
      {
        action: 'create',
        title: `${PREFIX} In New Folder ${Date.now()}`,
        folder: folderName,
      },
      'Folders',
    );
    ...
    assert.ok(
      elapsed < TIMEOUT_CREATE,
      `create in folder took ${elapsed}ms (>${TIMEOUT_CREATE}ms)`,
    );
    ...
    assert.ok(
      readText.includes(folderName),
      'note should be in the new folder',
    );
  });
});
...
describe('Notes Edge Cases', () => {
  ...
  it('E16: 200-char title', async () => {
    ...
    assert.ok(
      elapsed < TIMEOUT_CREATE,
      `long title create took ${elapsed}ms (>${TIMEOUT_CREATE}ms)`,
    );
  });
  ...
  it('E17: append exceeding 2000-char limit', async () => {
    ...
    const { text: createText } = await callTool(
      ...
      {
        action: 'create',
        title: `${PREFIX} Overflow ${Date.now()}`,
        body: 'X'.repeat(1900),
      },
      'Edge',
    );
    ...
    assert.ok(
      elapsed < TIMEOUT_READ,
      `append overflow took ${elapsed}ms (>${TIMEOUT_READ}ms)`,
    );
  });
  ...
  it('E18: search with no results', async () => {
    ...
    assert.ok(
      elapsed < TIMEOUT_SEARCH,
      `empty search took ${elapsed}ms (>${TIMEOUT_SEARCH}ms)`,
    );
  });
  ...
  it('E19: nonexistent note ID', async () => {
    const { text, elapsed } = await callTool(
      ...
      {
        action: 'read',
        id: 'x-coredata://FAKE-ID-DOES-NOT-EXIST/ICNote/p999999',
      },
      'Edge',
    );
    assert.ok(
    ...
    assert.ok(
      elapsed < TIMEOUT_READ,
      `bad ID read took ${elapsed}ms (>${TIMEOUT_READ}ms)`,
    );
  });
  ...
  it('E20: markdown-like content', async () => {
    ...
    const { text: createText } = await callTool(
      ...
      {
        action: 'create',
        title: `${PREFIX} Markdown ${Date.now()}`,
        body: mdBody,
      },
      'Edge',
    );
    ...
    assert.ok(
      elapsed < TIMEOUT_READ,
      `markdown read took ${elapsed}ms (>${TIMEOUT_READ}ms)`,
    );
  });
  ...
  it('E21: offset beyond total count', async () => {
    ...
    assert.strictEqual(
      itemCount,
      0,
      `expected 0 items at large offset, got ${itemCount}`,
    );
    assert.ok(
      elapsed < TIMEOUT_LIST,
      `large offset read took ${elapsed}ms (>${TIMEOUT_LIST}ms)`,
    );
  });
});
